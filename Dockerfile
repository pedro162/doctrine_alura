...
RUN sed -i 's!/var/www/html!/var/www/html/doctrine_alura/public!g' /etc/apache2/sites-available/000-default.conf
...
WORKDIR /var/www/html/doctrine_alura

# Verifica se o arquivo composer.json existe, se não, copia um arquivo de exemplo
COPY composer.json /var/www/html/doctrine_alura/composer.json

# Instale as dependências do Composer
RUN composer install --no-interaction || true

# Definir permissões adequadas para o diretório
RUN chown -R www-data:www-data /var/www/html/doctrine_alura
RUN chmod -R 755 /var/www/html/doctrine_alura
...
RUN useradd -ms /bin/bash appuser && \
    chown -R appuser:appuser /var/www/html/doctrine_alura
